...
if($stmt=$con->prepare('SELECT `Javascript` FROM `score_card` WHERE id=?')){
	...
	if($score>intval($htmlText)){
		if($stmt=$con->prepare('UPDATE `score_card` SET `Javascript` = ? WHERE `id`=?') ){
...
for ($i=0; $i <count($questionArr) ;$i++ ) {
	if($stmt=$con->prepare('SELECT question,explaination FROM Javascript_questions WHERE id=?')){
