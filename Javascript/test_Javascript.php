...
$stmt = $con->prepare('SELECT COUNT(id) FROM Javascript_questions');
$stmt->execute();
...
<div  id="full"><button  class="w3-button w3-red w3-hover-green" onclick="openFullscreen()">Allow full sreen</button> </div>
	<div class="w3-container w3-teal" id="header">
		<div class="w3-row">
			<div class="w3-col s6 w3-xlarge"><b>JavascriptTest </b></div>
		...
		<div class="w3-container w3-padding w3-margin" id="test">
			<form action="result_Javascript.php" method="POST" name="Test">
				<?php 
					$trackQN=0;
					for ($i=rand(1,5);$i<=$no_of_questions; $i=$i+rand(1,floor($no_of_questions/30))) { 
						$qid.=$i;					
						$stmt=$con->prepare('SELECT `question` FROM `Javascript_questions` WHERE id=?');
						$stmt->bind_param("s",$qid);
						$stmt->execute();
						$stmt->bind_result($question[$qid]);
						$stmt->fetch();
						$stmt->close();
						$rse_stmt=$con->prepare('SELECT `answer` FROM `Javascript_questions` WHERE id=?');
