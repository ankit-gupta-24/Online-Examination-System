...
if($stmt=$con->prepare('SELECT `html` FROM `score_card` WHERE id=?')){
	...
	if($score>intval($htmlText)){
		if($stmt=$con->prepare('UPDATE `score_card` SET `html` = ? WHERE `id`=?') ){
...
for ($i=0; $i <count($questionArr) ;$i++ ) {
	if($stmt=$con->prepare('SELECT question,explaination FROM html_questions WHERE id=?')){
