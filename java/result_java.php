...
if($stmt=$con->prepare('SELECT `java` FROM `score_card` WHERE id=?')){
	...
	if($score>intval($htmlText)){
		if($stmt=$con->prepare('UPDATE `score_card` SET `java` = ? WHERE `id`=?') ){
...
for ($i=0; $i <count($questionArr) ;$i++ ) {
	if($stmt=$con->prepare('SELECT question,explaination FROM java_questions WHERE id=?')){
